...
import { AdminDemoPanels } from "@/widgets/admin/AdminDemoPanels";
import {
...
function backendToSmartQuote(record: QuoteRequestRecordLite): SmartQuoteRequest {
  return {
    ...
    marketingOptIn: record.connectionAnswers?.marketingOptIn,
...
function estimateImageCard(
  ...
  ctx.fillText("DANNIEL CUERVO", 70, 120);
...
async function shareImageIfPossible(dataUrl: string, filename: string, text: string) {
  ...
  if (
    ...
    await navigator.share({
      title: "Cotizacion Danniel Cuervo",
      text,
      files: [file],
    });
    return true;
...
export function DannielAdminDashboard() {
  ...
  const adjustAndSendWhatsApp = async (quote: SmartQuoteRequest) => {
    ...
    const shortMessage = `Hola ${quote.clientName}, espero que estes muy bien. Te comparto tu cotizacion profesional de Danniel Cuervo: ${sessionsCount} sesiones aprox con un valor de ${money.format(sessionPrice)} por sesion (total ${money.format(total)}). Quedo atento para agendarte.`;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-3 grid gap-2 rounded-lg border border-white/10 bg-black/30 p-3 text-xs text-zinc-300 sm:grid-cols-2">
                            ...
                            {quote.connectionValues || typeof quote.marketingOptIn === "boolean" ? (
                              <p>
                                Marketing:{" "}
                                <span className="text-zinc-100">
                                  {quote.connectionValues ??
                                    (quote.marketingOptIn
                                      ? "Sí, quiere novedades"
                                      : "No optó por novedades")}
                                </span>
                              </p>
                            ) : null}
